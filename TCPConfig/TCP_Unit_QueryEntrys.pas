...
End;}
    ...
    Begin
         ...
         Begin
             ...
             // PrÅfen, ob der Verzeichniss Name bereits in der ListBox vorghanden ist
             NameIndex:=DebugForm.lbNFSDirectory.items.Indexof(DirName);
             // Name vorhanden ?
             IF NameIndex=-1 Then
             Begin
                  Test:=DebugForm.lbnfsDirectory.items.count;
                  // Name nicht gefunden, dann Zeilen ZÑhler der Datei EXPORTS verwenden
                  NFSReadListArray [DebugForm.lbNFsDirectory.items.count].Add(DebugStr);
             End else
             Begin
                  // Name bereits vorhanden, hinzufÅgen bei jeweiliger Position
...
{…ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕª
 ...
 VAR
  ...
  End;
                ...
                // ôffentliches Verzeichniss - ohne ALIAS ohne HOSTNAMEN und ohne Kommentare
                Begin
                ...
                Begin
                        ...
                        End else
                        Begin
                                // Sollte keiner vorhanden sein, listbox trotzdem fÅllen
                      ...
                      Begin
                        ...
                        DebugForm.lbNFSRights.items.add('H');  // Wenn keine Rechte angegeben wurden , kînnen auch keine HOSTS Vorhanden sein
                        LastPos:=LastPos+3;
                        DebugForm.lbNFSComment.items.add(QueryComments(LastPos));
  ...
  Begin
        CreateStringLists;
        Config_Line_Counter:=0; // Zeilen ZÑhler der Datei;
        Repeat
...
End;
        ...
        Begin
              Counter:=1;Result:=False;
              Repeat
                   //IF System.POS('#',TEST)>1 Then exit; // PrÅfen, ob wir nicht auf ein kommentar sto·en welches das wort "puplic" enthÑlt.
                   Test:=COpy(ConfigStr,counter,length(StringToSearch));
                   IF copy(ConfigStr,counter,length(StringToSearch))=StringToSearch Then
                   Begin
                        Result:=True;Pos:=Counter;Exit;
...
GeÑndert 06.10.2006 - aStrings.count lieferte immer 2 eintrÑge zurÅck, obwohl nur 1 Eintrag vorhanden war
GerÑndert 01.11.2006 - Umabu auf OS/2 INi File
}
VAR
...
VAR
   ...
   AliasLoopCounter:Byte; {SchleifenzÑhler fÅr Alias IP Schleife}
   S         :String;
...
{06.10.2006  QueryAmountInterfaces lieferte 2 aktive Interfaces zurÅck obwphl nur 1 installiert war.
